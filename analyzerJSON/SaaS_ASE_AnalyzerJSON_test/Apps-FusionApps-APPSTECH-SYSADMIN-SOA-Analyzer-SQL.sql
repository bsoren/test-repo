REM JC SQL Script Parameters: Apps-FusionApps-APPSTECH-SYSADMIN-SOA-Analyzer-SQL.sql "P_SR_NUMBER=<VALUE>;P_PURPOSE=<VALUE>;P_MAX_OUTPUT_ROWS=<VALUE>;P_DEBUG_MODE=<VALUE>;P_JOB_ID=<VALUE>;P_SQL_AT_TYPE=<VALUE>"
...
REM $Id: Apps-FusionApps-APPSTECH-SYSADMIN-SOA-Analyzer-SQL.sql, 200.10 2025/02/04 11:17:57 ccdumitr Exp $
REM +===========================================================================+
...
REM |                                                                           |
REM | FILENAME                                                                  |
REM |    Apps-FusionApps-APPSTECH-SYSADMIN-SOA-Analyzer-SQL.sql                 |
REM |                                                                           |
REM | DESCRIPTION                                                               |
REM |    SQL to run the Fusion SaaS Service Oriented Architecture (SOA) Analyzer|
REM +===========================================================================+
...
prompt ;
prompt Fusion SaaS Service Oriented Architecture (SOA) Analyzer is running...;
prompt ;
execute dbms_application_info.set_client_info('Apps-FusionApps-APPSTECH-SYSADMIN-SOA-Analyzer-SQL.sql 200.10');

variable b_file_name varchar2(400);
execute :b_file_name := 'SYSADMIN-SOA_Analyzer_';
...
begin
    l_doc_id := regexp_substr('2849066.1', '\\d+');

    l_azr := new json_object_t();
    l_azr.put('analyzer_id', '4909');
    l_azr.put('title', 'Fusion SaaS Service Oriented Architecture (SOA) Analyzer');
    l_azr.put('name', 'Apps-FusionApps-APPSTECH-SYSADMIN-SOA-Analyzer-SQL.sql');
    l_azr.put('analyzer_version', '200.10');
...
g_mos_doc_url     VARCHAR2(500) :=
  'https://mosemp.us.oracle.com/epmos/faces/DocumentDisplay?parent=ANALYZER&sourceId=2849066.1';
...
procedure set_trace_module(
    ...
    l_module varchar2(100) := 'analyzer:4909';
begin
...
BEGIN
    ...
    print_out('<TITLE>SYSADMIN-SOA Analyzer Report</TITLE>');
...
function updateInternalViewer() {
    if (window.location.href.indexOf("oracle.com/iss/") >= 0 ||
        ...
        $("div#feedback").find("a.blacklink").attr("href", "https://mosemp.us.oracle.com/epmos/faces/DocumentDisplay?id=2849066.1#feedback");
...
BEGIN
    ...
    l_url := l_url ||   -- base URL
        utl_url.escape( -- Escape invalid URL characters
            ...
            '200.10' || l_run_delim || -- analyzer version
            l_sr_text || -- SR number
...
BEGIN
    ...
    <div style="display: none;">
        ...
        <span id="feedback-azr">4909</span>
...
PROCEDURE validate_parameters(
           ...
           ,PURPOSE                        IN VARCHAR2    DEFAULT 'Fusion SaaS Service Oriented Architecture (SOA) Analyzer'
...
-- Revision and date values can be populated by RCS
  l_date_char := rtrim(replace('$Date: 2025/02/04 11:17:57 $','$',''));
...
-- Do not report host details (EBSAF-274); reverted by EBSAF-293
  ...
  g_rep_info('File Name') := 'Apps-FusionApps-APPSTECH-SYSADMIN-SOA-Analyzer-SQL.sql';
  g_rep_info('File Version') := '200.10';
  g_rep_info('Framework Version') := RTRIM(g_framework_version, ' ');
  g_rep_info('Execution Date') := to_char(sysdate,'DD-MON-YYYY HH24:MI:SS');
  g_rep_info('Description') := expand_html('The ' || analyzer_title || ' [2849066.1] is a self-service health-check script that reviews the overall footprint, analyzes current configurations and settings for the environment and provides feedback and recommendations on best practices. Your application data is not altered in any way when you run this analyzer.');
...
debug('begin add_signature: APPSTECH_SYSADMIN_SOA_GENERIC_POD_DB_INFO');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '21615',
      p_sig_id                 => 'APPSTECH_SYSADMIN_SOA_GENERIC_POD_DB_INFO',
      p_sig_sql                => 'select a.Release_Version_Number, a.Release_Version_Name,
b.POD_NAME,b.LAST_UPGRADE_DATE,
d.DB_RELEASE_VERSION from
(select EXTERNAL_VERSION Release_Version_Number,
NAME Release_Version_Name from
FUSION.ask_release_versions Where CURRENT_RELEASE_FLAG=''Y'') a,
(select distinct EXTERNAL_VIRTUAL_HOST POD_NAME,TO_CHAR(LAST_UPDATE_DATE, ''MM-DD-YYYY'') "LAST_UPGRADE_DATE" from FUSION.ASK_DEPLOYED_DOMAINS where DEPLOYED_DOMAIN_NAME = ''FADomain'') b, (SELECT DISTINCT version DB_RELEASE_VERSION FROM V$INSTANCE) d',
      p_title                  => 'Fusion POD and DB information',
      p_fail_condition         => 'NRS',
      p_problem_descr          => 'This is not a problem, this is to show generic POD and Database information.',
      p_solution               => 'This is not a problem, this is to show generic POD and Database information.',
      p_success_msg            => 'Following are POD and DB details.',
   ...
   l_info.delete;
debug('end add_signature: APPSTECH_SYSADMIN_SOA_GENERIC_POD_DB_INFO');



debug('begin add_signature: APPSTECH_SYSADMIN_SOA_GENERIC_WHITELISTED_IP_LBAC');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '21618',
      p_sig_id                 => 'APPSTECH_SYSADMIN_SOA_GENERIC_WHITELISTED_IP_LBAC',
      p_sig_sql                => 'SELECT 
LEVEL_NAME,
...
FROM fusion.FND_PROFILE_OPTIONS_B
WHERE PROFILE_OPTION_NAME =''FND_IP_WHITELIST_HYBRID_MODE'')
AND PROFILE_OPTION_VALUE = ''Y'' AND LEVEL_NAME = ''SITE''',
      p_title                  => 'IP list Whitelisted in LBAC',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'Get status of LBAC',
      p_solution               => 'LBAC is enabled',
      p_success_msg            => 'LBAC is not enabled',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('W','W'),
   ...
   l_info.delete;
debug('end add_signature: APPSTECH_SYSADMIN_SOA_GENERIC_WHITELISTED_IP_LBAC');



debug('begin add_signature: APPSTECH_SYSADMIN_SOA_AGG_FYI_DANGLING');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '20602',
      p_sig_id                 => 'APPSTECH_SYSADMIN_SOA_AGG_FYI_DANGLING',
      p_sig_sql                => 'SELECT COUNT(*) FROM FA_FUSION_SOAINFRA.WFTASK T1 
WHERE T1.ROOTTASKID IN (SELECT T2.TASKID 
FROM FA_FUSION_SOAINFRA.WFTASK T2 
WHERE T2.ROOTTASKID =T2.TASKID AND T2.STATE IS NULL) 
AND T1.WORKFLOWPATTERN =  ''AGGREGATION'' 
AND T1.STATE = ''ASSIGNED''',
      p_title                  => 'Check Dangling Aggregate FYI tasks assigned',
      p_fail_condition         => 'NRS',
      p_problem_descr          => 'Check Aggregated FYI tasks in assigned state for roottask completed or null',
      p_solution               => 'Analyze sample task and work with Development in getting datafix to update dangling tasks.',
      p_success_msg            => 'No dangling FYI aggregated tasks ',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('I','W'),
      p_print_sql_output       => nvl('Y','RS'),
      p_limit_rows             => nvl('Y','Y'),
   ...
   l_info.delete;
debug('end add_signature: APPSTECH_SYSADMIN_SOA_AGG_FYI_DANGLING');



debug('begin add_signature: APPSTECH_SYSADMIN_SOA_WFTASK_RETRY');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '21619',
      p_sig_id                 => 'APPSTECH_SYSADMIN_SOA_WFTASK_RETRY',
      p_sig_sql                => 'select count(*) task_retry from FA_FUSION_SOAINFRA.WFTASK where state = ''ASSIGNED'' and substate = ''RETRY''',
      p_title                  => 'Task in Retry status',
      p_fail_condition         => 'NRS',
      p_problem_descr          => 'Check for workflow tasks stuck in retry status. ',
      p_solution               => 'Analyze data and composite instance for tasks stuck in retry status.',
      p_success_msg            => 'No tasks stuck in retry status.',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('I','W'),
      p_print_sql_output       => nvl('Y','RS'),
      p_limit_rows             => nvl('Y','Y'),
   ...
   l_info.delete;
debug('end add_signature: APPSTECH_SYSADMIN_SOA_WFTASK_RETRY');



debug('begin add_signature: APPSTECH_SYSADMIN_SOA_COMPOSITE_DN');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '21620',
      p_sig_id                 => 'APPSTECH_SYSADMIN_SOA_COMPOSITE_DN',
      p_sig_sql                => 'select trunc(created_time), count(1) composite_dn from fa_fusion_soainfra.composite_instance where trunc(created_time)>= trunc(sysdate) - 7 group by trunc(created_time)',
      p_title                  => 'SOA Composite count ',
      p_fail_condition         => 'NRS',
      p_problem_descr          => 'SOA Composite count.
',
      p_solution               => 'This Signature will give count of SOA composites created for last 7 days.',
      p_success_msg            => '',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('I','W'),
      p_print_sql_output       => nvl('Y','RS'),
   ...
   l_info.delete;
debug('end add_signature: APPSTECH_SYSADMIN_SOA_COMPOSITE_DN');



debug('begin add_signature: APPSTECH_SYSADMIN_SOA_BRANDINGLOGO');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '21621',
      p_sig_id                 => 'APPSTECH_SYSADMIN_SOA_BRANDINGLOGO',
      p_sig_sql                => 'select /*Analyser*/  PREFERENCENAME, PREFERENCEVALUE  from FA_FUSION_SOAINFRA.wfuserpreference where PREFERENCENAME = ''brandingLogo''',
      p_title                  => 'BPM Worklist branding logo',
      p_fail_condition         => 'NRS',
      p_problem_descr          => 'Check BPM Worklist page logo path is correct.',
      p_solution               => '[2846969.1]BPM Worklist Page Default Branding Logo not Displayed after Upgrading to Fusion 22A ',
      p_success_msg            => 'BPM Worklist Page Default Branding Logo path is correct.',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('I','W'),
      p_print_sql_output       => nvl('Y','RS'),
      p_limit_rows             => nvl('Y','Y'),
   ...
   l_info.delete;
debug('end add_signature: APPSTECH_SYSADMIN_SOA_BRANDINGLOGO');



debug('begin add_signature: APPSTECH_SYSADMIN_SOA_BPM_REMINDER_PREFIX');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '21623',
      p_sig_id                 => 'APPSTECH_SYSADMIN_SOA_BPM_REMINDER_PREFIX',
      p_sig_sql                => 'SELECT  PRF.PROFILE_OPTION_NAME, VAL.LEVEL_NAME, VAL.LEVEL_VALUE, VAL.PROFILE_OPTION_VALUE FROM FUSION.FND_PROFILE_OPTION_VALUES VAL, FUSION.FND_PROFILE_OPTIONS PRF WHERE PRF.PROFILE_OPTION_NAME = ''BPM_REMINDER_PREFIX'' AND VAL.PROFILE_OPTION_ID = PRF.PROFILE_OPTION_ID',
      p_title                  => 'Workflow Notifcation Reminder Prefix',
      p_fail_condition         => 'NRS',
      p_problem_descr          => 'Display Reminder Prefix for Approval Notifications',
      p_solution               => 'Reminder Prefix can be configured by specifying a value for the Add Reminder Prefix to Notification Title (BPM_REMINDER_PREFIX) profile option.
...
',
      p_success_msg            => 'By default Reminder Prefix is enabled.',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('I','W'),
      p_print_sql_output       => nvl('Y','RS'),
      p_limit_rows             => nvl('Y','Y'),
   ...
   l_info.delete;
debug('end add_signature: APPSTECH_SYSADMIN_SOA_BPM_REMINDER_PREFIX');



debug('begin add_signature: APPSTECH_SYSADMIN_SOA_FYI_EXPIRATION_PERIOD');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '21624',
      p_sig_id                 => 'APPSTECH_SYSADMIN_SOA_FYI_EXPIRATION_PERIOD',
      p_sig_sql                => 'SELECT PRF.PROFILE_OPTION_NAME, VAL.LEVEL_NAME, VAL.LEVEL_VALUE, VAL.PROFILE_OPTION_VALUE FROM FUSION.FND_PROFILE_OPTION_VALUES VAL, FUSION.FND_PROFILE_OPTIONS PRF WHERE PRF.PROFILE_OPTION_NAME = ''FND_NOTIFICATION_EXP_PERIOD'' AND VAL.PROFILE_OPTION_ID = PRF.PROFILE_OPTION_ID',
      p_title                  => 'FYI task Expiration period',
      p_fail_condition         => 'NRS',
      p_problem_descr          => 'Verify FYI notifications expiration period.',
      p_solution               => '[2449251.1]Automatically Expire Outstanding FYI BPM Tasks',
      p_success_msg            => 'FYI Notifications Expiration Period is set to default 7 days.',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('I','W'),
      p_print_sql_output       => nvl('Y','RS'),
      p_limit_rows             => nvl('Y','Y'),
   ...
   l_info.delete;
debug('end add_signature: APPSTECH_SYSADMIN_SOA_FYI_EXPIRATION_PERIOD');



debug('begin add_signature: APPSTECH_SYSADMIN_SOA_GENERIC_TABLESPACE');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '21625',
      p_sig_id                 => 'APPSTECH_SYSADMIN_SOA_GENERIC_TABLESPACE',
      p_sig_sql                => 'select df.tablespace_name "Tablespace",
...
where df.tablespace_name = tu.tablespace_name',
      p_title                  => 'Tablespace details',
      p_fail_condition         => 'NRS',
      p_problem_descr          => 'Tablespace details',
      p_solution               => 'Tablespace details',
      p_success_msg            => 'Tablespace details',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('I','W'),
      p_print_sql_output       => nvl('Y','RS'),
      p_limit_rows             => nvl('N','Y'),
   ...
   l_info.delete;
debug('end add_signature: APPSTECH_SYSADMIN_SOA_GENERIC_TABLESPACE');



debug('begin add_signature: APPSTECH_SYSADMIN_SOA_GENERIC_TEMPSPACE');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '21626',
      p_sig_id                 => 'APPSTECH_SYSADMIN_SOA_GENERIC_TEMPSPACE',
      p_sig_sql                => 'select file_name, tablespace_name, Round(bytes*(9.31/10000000000), 4) USED_GB,
Round(maxbytes*(9.31/10000000000), 4)MAX_GB,
round((bytes/maxbytes), 2)*100 Usage_percentage from dba_temp_files',
      p_title                  => 'Tempspace details',
      p_fail_condition         => 'NRS',
      p_problem_descr          => 'Tempspace details',
      p_solution               => 'Tempspace detail',
      p_success_msg            => 'Tempspace detail',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('I','W'),
      p_print_sql_output       => nvl('Y','RS'),
      p_limit_rows             => nvl('N','Y'),
   ...
   l_info.delete;
debug('end add_signature: APPSTECH_SYSADMIN_SOA_GENERIC_TEMPSPACE');



debug('begin add_signature: APPSTECH_SYSADMIN_SOA_AFLOG_CHECK');
  ...
  add_signature(
      p_sig_repo_id            => '21627',
      p_sig_id                 => 'APPSTECH_SYSADMIN_SOA_AFLOG_CHECK',
      p_sig_sql                => 'select
    /*Analyzer*/
    level_name,
...
',
   ...
   l_info.delete;
debug('end add_signature: APPSTECH_SYSADMIN_SOA_AFLOG_CHECK');



debug('begin add_signature: FA_APPSTECH_SA_ATK_SYSADMIN_SOA_WFTASK_COUNT');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '22136',
      p_sig_id                 => 'FA_APPSTECH_SA_ATK_SYSADMIN_SOA_WFTASK_COUNT',
      p_sig_sql                => 'SELECT /*Analyzer*/ count(tasknumber) FROM fa_fusion_soainfra.wftask',
      p_title                  => 'SOA Wftask Count',
      p_fail_condition         => '[count(tasknumber)] > [1000000]',
      p_problem_descr          => 'SOA Wftask Count',
      p_solution               => 'Verify wftask count is within acceptable limit.',
      p_success_msg            => 'Wftask count within limit.',
   ...
   l_info.delete;
debug('end add_signature: FA_APPSTECH_SA_ATK_SYSADMIN_SOA_WFTASK_COUNT');



debug('begin add_signature: APPSTECH_SYSADMIN_SOA_UMS_DRIVER');
   l_info('##MASK##CONTENT'):= 'DISPLAY_FIRST_25_PCNT';
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '18730',
      p_sig_id                 => 'APPSTECH_SYSADMIN_SOA_UMS_DRIVER',
      p_sig_sql                => 'SELECT  status.message_id,
        TO_CHAR(status.status_date,''DD-MON-YYYY HH24:MI:SS'') message_date,
        status.type status,
        status.content
FROM    fusion_orasdpm.delivery_attempt delivery,
        fusion_orasdpm.address address,
        fusion_orasdpm.status status
WHERE   delivery.address_id  = address.addr_id
        AND status.message_id  = delivery.mid
        AND status.type  = ''DELIVERY_TO_GATEWAY_FAILURE''
        AND address.delivery_type=''EMAIL''
        AND status.status_date   > sysdate - 7
ORDER BY message_date DESC',
      p_title                  => 'Verify Email delivered to the UMS Gateway status',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'Check SOA Workflow notifications failed to deliver to the UMS gateway driver in last 7 days',
      p_solution               => '<a href="https://fa-etmi-saasfaprod1.fa.ocs.oraclecloud.com/fscmUI/redwood/myknowledge/content/container/main/article?answerId=10056047"_blank">KB160766 Internal Troubleshooting for Fusion Applications Notifications: No Worklist Email is Received</a> ',
      p_success_msg            => 'No notifications failed within last 7 days',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('W','W'),
      p_print_sql_output       => nvl('Y','RS'),
      p_limit_rows             => nvl('Y','Y'),
   ...
   l_info.delete;
debug('end add_signature: APPSTECH_SYSADMIN_SOA_UMS_DRIVER');



debug('begin add_signature: APPSTECH_SYSADMIN_SOA_NOTIF_DAYS');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '18921',
      p_sig_id                 => 'APPSTECH_SYSADMIN_SOA_NOTIF_DAYS',
      p_sig_sql                => 'SELECT /*Analyzer*/ TO_CHAR(createdtime, ''DD-MON-YYYY'') as the_day,COUNT(ID),type,status
FROM FA_FUSION_SOAINFRA.bpelnotification
GROUP BY TO_CHAR(createdtime, ''DD-MON-YYYY''),type,status
ORDER BY 1 DESC',
      p_title                  => 'Verify the notification count/days from bpelnotification',
      p_fail_condition         => 'NRS',
      p_problem_descr          => 'check the bpelnotification table count of notifications',
      p_solution               => '[1960637.1] Actionable Email Approvals not Reflected in Fusion Applications',
      p_success_msg            => 'Notification count status:',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('I','W'),
      p_print_sql_output       => nvl('Y','RS'),
      p_limit_rows             => nvl('Y','Y'),
   ...
   l_info.delete;
debug('end add_signature: APPSTECH_SYSADMIN_SOA_NOTIF_DAYS');



debug('begin add_signature: APPSTECH_SYSADMIN_SOA_INCOMING_MSG');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '19100',
      p_sig_id                 => 'APPSTECH_SYSADMIN_SOA_INCOMING_MSG',
      p_sig_sql                => 'select MESSAGE_CHANNEL, RECEIVED_DATE,  COMPOSITE_INSTANCEID, COMPONENT_NAME  from fa_fusion_soainfra.BPELNotification_filtermsgs 
where received_date > sysdate -4',
      p_title                  => 'Check for incoming messages ',
      p_fail_condition         => 'NRS',
      p_problem_descr          => 'Actionable emails are not reflected in Fusion',
      p_solution               => '[1960637.1] Actionable Email Approvals not Reflected in Fusion Applications',
      p_success_msg            => 'Details of the incoming messages',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('I','W'),
      p_print_sql_output       => nvl('Y','RS'),
      p_limit_rows             => nvl('Y','Y'),
   ...
   l_info.delete;
debug('end add_signature: APPSTECH_SYSADMIN_SOA_INCOMING_MSG');



debug('begin add_signature: APPSTECH_SYSADMIN_SOA_SYNC_BELL');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '18815',
      p_sig_id                 => 'APPSTECH_SYSADMIN_SOA_SYNC_BELL',
      p_sig_sql                => 'select
/*Analyser*/
requestid,
submission,
processstart,
DECODE(state,1,''Wait'', 11,''Warning'',2,''Ready'', 12,''Succeeded'',
...
9,''Canceled'', 19,''Error Manual Recovery'',10,''Error'', state) status,
definition
FROM FUSION.ESS_REQUEST_HISTORY
where definition like ''%SyncBellNotifications%'' 
and (processstart > (SELECT sysdate-6/24 FROM dual))
and state = 12
order by 2,3 desc',
      p_title                  => 'Verify if SyncBellNotifications ess job is running',
      p_fail_condition         => 'NRS',
      p_problem_descr          => 'Bell icon showing wrong data',
      p_solution               => '[1598259.1] Fusion Applications: Notification Frequently Asked Questions (FAQ''s) , section Global Header Bell Icon',
      p_success_msg            => 'Bell icon showing current information',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('W','W'),
      p_print_sql_output       => nvl('Y','RS'),
      p_limit_rows             => nvl('Y','Y'),
   ...
   l_info.delete;
debug('end add_signature: APPSTECH_SYSADMIN_SOA_SYNC_BELL');



debug('begin add_signature: FA_APPSTECH_SA_APPSTECH_SYSADMIN_SOA_ORPHAN_TASK_PART1');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '30420',
      p_sig_id                 => 'FA_APPSTECH_SA_APPSTECH_SYSADMIN_SOA_ORPHAN_TASK_PART1',
      p_sig_sql                => 'SELECT count(*)
FROM FA_FUSION_SOAINFRA.WFTASK T1
WHERE T1.STATE = ''ASSIGNED''
AND NOT EXISTS
(SELECT TASKID FROM FA_FUSION_SOAINFRA.WFTASK T2
WHERE T2.TASKID=T1.ROOTTASKID)',
      p_title                  => 'Orphan Tasks in WFTASK',
      p_fail_condition         => 'count > 0',
      p_problem_descr          => 'Determine if orphaned tasks in FA_FUSION_SOAINFRA.WFTASK.',
      p_solution               => 'SQLs returns greater than 0, a datafix may be needed for orphaned tasks found in FA_FUSION_SOAINFRA.WFTASK.',
      p_success_msg            => 'No orphan tasks in FA_FUSION_SOAINFRA.WFTASK.',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('W','W'),
      p_print_sql_output       => nvl('Y','RS'),
      p_limit_rows             => nvl('Y','Y'),
   ...
   l_info.delete;
debug('end add_signature: FA_APPSTECH_SA_APPSTECH_SYSADMIN_SOA_ORPHAN_TASK_PART1');



debug('begin add_signature: FA_APPSTECH_SA_APPSTECH_SYSADMIN_SOA_ORPHAN_TASK_PART2');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '30448',
      p_sig_id                 => 'FA_APPSTECH_SA_APPSTECH_SYSADMIN_SOA_ORPHAN_TASK_PART2',
      p_sig_sql                => 'SELECT count(*)
FROM FA_FUSION_SOAINFRA.WFTASK_SEC T1
WHERE T1.STATE = ''ASSIGNED''
AND NOT EXISTS
(SELECT TASKID FROM FA_FUSION_SOAINFRA.WFTASK_SEC T2
WHERE T2.TASKID=T1.ROOTTASKID)',
      p_title                  => 'Orphan Tasks in WFTASK_SEC',
      p_fail_condition         => 'count > 0',
      p_problem_descr          => 'Determine if orphaned tasks in FA_FUSION_SOAINFRA.WFTASK_SEC.',
      p_solution               => 'SQLs returns greater than 0, a datafix may be needed for orphaned tasks found in FA_FUSION_SOAINFRA.WFTASK_SEC.',
      p_success_msg            => 'No orphan tasks in FA_FUSION_SOAINFRA.WFTASK_SEC.',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('W','W'),
      p_print_sql_output       => nvl('Y','RS'),
      p_limit_rows             => nvl('Y','Y'),
   ...
   l_info.delete;
debug('end add_signature: FA_APPSTECH_SA_APPSTECH_SYSADMIN_SOA_ORPHAN_TASK_PART2');



debug('begin add_signature: FA_APPSTECH_SA_ATK_SYSADMIN_SOA_INCOMING_EMAIL_ALERT1');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '27679',
      p_sig_id                 => 'FA_APPSTECH_SA_ATK_SYSADMIN_SOA_INCOMING_EMAIL_ALERT1',
      p_sig_sql                => 'select  count(*)  
 from FA_FUSION_SOAINFRA.bpelnotification_filtermsgs
where RECEIVED_DATE > sysdate -2
and MESSAGE_CHANNEL = ''email'' and filter_key = ''NOTIF_MSG.ResponseProcessed''

',
      p_title                  => 'Count incoming messages in UMS and HWF Alert 1',
      p_fail_condition         => 'NRS',
      p_problem_descr          => 'There may be an email driver/ums health problem since no incoming emails have been received for a period of time.',
      p_solution               => 'Count Incoming messages in HWF side  > 0 for last 2 days.
<br>
Count Incoming messages in UMS side > 0 for last 2 days.
 ',
      p_success_msg            => 'There are Incoming messages in UMS and HWF .',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('W','W'),
      p_print_sql_output       => nvl('Y','RS'),
      p_limit_rows             => nvl('Y','Y'),
   ...
   l_info.delete;
debug('end add_signature: FA_APPSTECH_SA_ATK_SYSADMIN_SOA_INCOMING_EMAIL_ALERT1');



debug('begin add_signature: FA_APPSTECH_SA_ATK_SYSADMIN_SOA_INCOMING_EMAIL_ALERT2');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '29477',
      p_sig_id                 => 'FA_APPSTECH_SA_ATK_SYSADMIN_SOA_INCOMING_EMAIL_ALERT2',
      p_sig_sql                => 'select  count(*) from fusion_orasdpm.message_sec where CREATION_DATE > sysdate -2

',
      p_title                  => 'Count incoming messages in UMS and HWF Alert 2',
      p_fail_condition         => 'NRS',
      p_problem_descr          => 'There may be an email driver/ums health problem since no incoming emails have been received for a period of time.',
      p_solution               => 'Count Incoming messages in HWF side  > 0 for last 2 days.
 <br>
Count Incoming messages in UMS side > 0 for last 2 days.
 ',
   ...
   l_info.delete;
debug('end add_signature: FA_APPSTECH_SA_ATK_SYSADMIN_SOA_INCOMING_EMAIL_ALERT2');



debug('begin add_signature: FA_APPSTECH_SA_ATK_SYSADMIN_SOA_INCOMING_EMAIL_ALERT3');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '29479',
      p_sig_id                 => 'FA_APPSTECH_SA_ATK_SYSADMIN_SOA_INCOMING_EMAIL_ALERT3',
      p_sig_sql                => 'select  count(*) from fusion_orasdpm.status_sec  where TYPE = ''INTERMEDIATE_NOTIFICATION'' and  status_date > sysdate -2 ',
      p_title                  => 'Count incoming messages in UMS and HWF Alert 3',
      p_fail_condition         => 'NRS',
      p_problem_descr          => 'There may be an email driver/ums health problem since no incoming emails have been received for a period of time.',
      p_solution               => 'Count Incoming messages in HWF side  > 0 for last 2 days.
<br>
Count Incoming messages in UMS side > 0 for last 2 days.
 ',
   ...
   l_info.delete;
debug('end add_signature: FA_APPSTECH_SA_ATK_SYSADMIN_SOA_INCOMING_EMAIL_ALERT3');



debug('begin add_signature: APPSTECH_SYSADMIN_SOA_EDN_QUEUE');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '18731',
      p_sig_id                 => 'APPSTECH_SYSADMIN_SOA_EDN_QUEUE',
      p_sig_sql                => 'select count(STATE), t.user_data.header.get_string_property (''EDN$namespace'') , t.user_data.header.get_string_property (''EDN$localName'') from FA_FUSION_SOAINFRA.EDN_AQJMS_TOPIC_TABLE t group by t.user_data.header.get_string_property (''EDN$namespace'') , t.user_data.header.get_string_property (''EDN$localName'') having  count(STATE) > 50000',
      p_title                  => 'Verify SOA EDN Queue Count',
      p_fail_condition         => '[count(STATE)] > [50000]',
      p_problem_descr          => 'Check EDN queue for any SOA performance',
      p_solution               => '<a href="https://docs.oracle.com/en/cloud/saas/human-resources/24d/ochus/best-practices-for-user-and-role-provisioning-in-hcm.html#s20079277">Best Practices for User and Role Provisioning in HCM</a>
',
      p_success_msg            => 'EDN queue counts are OK',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('W','W'),
      p_print_sql_output       => nvl('Y','RS'),
      p_limit_rows             => nvl('Y','Y'),
   ...
   l_info.delete;
debug('end add_signature: APPSTECH_SYSADMIN_SOA_EDN_QUEUE');



debug('begin add_signature: APPSTECH_SYSADMIN_SOA_WFTASK_STATS');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '19874',
      p_sig_id                 => 'APPSTECH_SYSADMIN_SOA_WFTASK_STATS',
      p_sig_sql                => 'select TABLE_NAME, OBJECT_TYPE, NUM_ROWS, LAST_ANALYZED, STALE_STATS
from dba_tab_statistics
where owner = ''FA_FUSION_SOAINFRA''
AND
TABLE_NAME IN (''WFTASK'', ''WFASSIGNEE'', ''WFTASK_TL'',''WFTASKHISTORY'')
and STALE_STATS=''YES''',
      p_title                  => 'Verify SOA Infra Wftask Stats gather',
      p_fail_condition         => 'RS',
      p_problem_descr          => 'Check wftask stats for any BPM Worklist Performance issues',
      p_solution               => 'Engage Performance Operations to review stale stats.',
      p_success_msg            => 'No Wftask Stale Stats.',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('W','W'),
      p_print_sql_output       => nvl('Y','RS'),
      p_limit_rows             => nvl('Y','Y'),
   ...
   l_info.delete;
debug('end add_signature: APPSTECH_SYSADMIN_SOA_WFTASK_STATS');



debug('begin add_signature: APPSTECH_SYSADMIN_SOA_WFTASK_STALE_COUNT');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '20601',
      p_sig_id                 => 'APPSTECH_SYSADMIN_SOA_WFTASK_STALE_COUNT',
      p_sig_sql                => 'SELECT COUNT(*) FROM FA_FUSION_SOAINFRA.WFTASK WHERE STATE=''STALE'' AND CREATEDDATE < SYSDATE - 180 and VERSIONREASON = ''Auto_Stale''',
      p_title                  => 'Check Stale tasks in wftask table',
      p_fail_condition         => 'NRS',
      p_problem_descr          => 'Check Stale tasks not purged and assigned in wftask table',
      p_solution               => 'Stale tasks should get purged after 180 days from updated date with weekly purge job running

Fusion Applications: SOA Purge Details of a Software as a Service (SaaS) Fusion Apps Environment[2175534.1]',
      p_success_msg            => 'No Stale tasks',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('I','W'),
      p_print_sql_output       => nvl('Y','RS'),
      p_limit_rows             => nvl('Y','Y'),
   ...
   l_info.delete;
debug('end add_signature: APPSTECH_SYSADMIN_SOA_WFTASK_STALE_COUNT');



debug('begin add_signature: FA_APPSTECH_SA_ATK_SYSADMIN_SOA_COMMON_FAULT');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '22078',
      p_sig_id                 => 'FA_APPSTECH_SA_ATK_SYSADMIN_SOA_COMMON_FAULT',
      p_sig_sql                => 'SELECT /*Analyzer*/ state,fault_type,fault_name,count(state) FROM fa_fusion_soainfra.SCA_COMMON_FAULT where trunc(CREATION_DATE) > trunc(sysdate) - 14 group by state,fault_type,fault_name',
      p_title                  => 'SOA Faulted Composites',
      p_fail_condition         => '[count(state)] > [2000]',
      p_problem_descr          => 'SOA Composites with System & Business faults',
      p_solution               => 'Check any high number of faults causing environment performance impact.',
      p_success_msg            => 'Faults within limit.',
      p_print_condition        => nvl('FAILURE','ALWAYS'),
      p_fail_type              => nvl('W','W'),
      p_print_sql_output       => nvl('RS','RS'),
      p_limit_rows             => nvl('N','Y'),
   ...
   l_info.delete;
debug('end add_signature: FA_APPSTECH_SA_ATK_SYSADMIN_SOA_COMMON_FAULT');



debug('begin add_signature: FA_APPSTECH_SA_ATK_SOA_FYI_DISMISS');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '25207',
      p_sig_id                 => 'FA_APPSTECH_SA_ATK_SOA_FYI_DISMISS',
      p_sig_sql                => 'SELECT 
    COUNT(WFN.TASKID) open_fyi_todo,
    WFN.WORKFLOWPATTERN
FROM
    FA_FUSION_SOAINFRA.WFTASK WFN
WHERE
        WFN.STATE = ''ASSIGNED''
    AND WFN.CREATEDDATE < sysdate - 7
    AND WFN.UPDATEDDATE < sysdate - 7
    AND ( WFN.WORKFLOWPATTERN = ''FYI''
          OR WFN.TASKNAMESPACE = ''http://xmlns.oracle.com/bpel/workflow/taskDefinition/DefaultToDoTaskDefinition'' )
GROUP BY
    WFN.WORKFLOWPATTERN',
      p_title                  => 'Count of FYI and ToDo tasks that should be auto-dismissed',
      p_fail_condition         => 'NRS',
      p_problem_descr          => 'Count of FYI and ToDo tasks that should be auto-dismissed',
      p_solution               => 'FYI Auto Dismissal Not Working
https://confluence.oraclecorp.com/confluence/display/SE/HWF+SE+Knowledge+Base#HWFSEKnowledgeBase-CheckForPurgeJobs',
      p_success_msg            => 'There are not FYI and ToDo tasks that should be dismissed',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('I','W'),
      p_print_sql_output       => nvl('Y','RS'),
      p_limit_rows             => nvl('Y','Y'),
   ...
   l_info.delete;
debug('end add_signature: FA_APPSTECH_SA_ATK_SOA_FYI_DISMISS');



debug('begin add_signature: FA_APPSTECH_SA_ATK_SYSADMIN_SOA_ARCHIVED_NOT_PURGED');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '25351',
      p_sig_id                 => 'FA_APPSTECH_SA_ATK_SYSADMIN_SOA_ARCHIVED_NOT_PURGED',
      p_sig_sql                => 'select count(*) view_archived 
from FA_FUSION_SOAINFRA.wftask_view
where archivedon is not NULL',
      p_title                  => 'Count of tasks archived but not yet purged.',
      p_fail_condition         => 'NRS',
      p_problem_descr          => 'Count of tasks archived but not purged.',
      p_solution               => 'Archived tasks that are not yet purged:
https://confluence.oraclecorp.com/confluence/display/SE/HWF+Performance+and+Health',
      p_success_msg            => 'There are no archived tasks that are not purged.',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('I','W'),
      p_print_sql_output       => nvl('Y','RS'),
      p_limit_rows             => nvl('Y','Y'),
   ...
   l_info.delete;
debug('end add_signature: FA_APPSTECH_SA_ATK_SYSADMIN_SOA_ARCHIVED_NOT_PURGED');



debug('begin add_signature: FA_APPSTECH_SA_ATK_SYSADMIN_SOA_N_ARCH');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '25542',
      p_sig_id                 => 'FA_APPSTECH_SA_ATK_SYSADMIN_SOA_N_ARCH',
      p_sig_sql                => 'select count(tasknumber) from FA_FUSION_SOAINFRA.wftask_view
where archivedon is NULL',
      p_title                  => 'Tasks available for archive but not yet archived',
      p_fail_condition         => '[count(tasknumber)] > [1000000]',
      p_problem_descr          => 'Tasks available for archive but not yet archived',
      p_solution               => 'Tasks available for archive but not yet archived',
      p_success_msg            => 'Tasks available for archive but not yet archived',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('W','W'),
      p_print_sql_output       => nvl('Y','RS'),
      p_limit_rows             => nvl('Y','Y'),
   ...
   l_info.delete;
debug('end add_signature: FA_APPSTECH_SA_ATK_SYSADMIN_SOA_N_ARCH');



debug('begin add_signature: FA_APPSTECH_SA_ATK_SYSADMIN_SOA_COUNT_PURGE');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '25545',
      p_sig_id                 => 'FA_APPSTECH_SA_ATK_SYSADMIN_SOA_COUNT_PURGE',
      p_sig_sql                => 'SELECT 
    COUNT(task.taskid) purge_180
FROM
    FA_FUSION_SOAINFRA.wftask task
WHERE
    ( task.state IS NULL
      OR task.state IN ( ''DELETED'', ''ERRORED'', ''EXPIRED'', ''STALE'', ''WITHDRAWN'' ) )
    AND task.createddate < sysdate - 180
    AND task.updateddate < sysdate - 180
    AND NOT EXISTS (
        SELECT
            task2.parenttaskid
        FROM
            FA_FUSION_SOAINFRA.wftask task2
        WHERE
            task.taskid = task2.parenttaskid
            AND task2.state IN ( ''ASSIGNED'', ''ALERTED'', ''INFO_REQUESTED'', ''OUTCOME_UPDATED'', ''SUSPENDED'' )
    )
    AND task.systemstring1 IS NOT NULL',
      p_title                  => 'Count eligible for purge disregarding composite state',
      p_fail_condition         => 'NRS',
      p_problem_descr          => 'Count eligible for purge disregarding composite state',
      p_solution               => 'Count eligible for purge disregarding composite state',
      p_success_msg            => 'Count eligible for purge disregarding composite state',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('I','W'),
      p_print_sql_output       => nvl('Y','RS'),
      p_limit_rows             => nvl('Y','Y'),
   ...
   l_info.delete;
debug('end add_signature: FA_APPSTECH_SA_ATK_SYSADMIN_SOA_COUNT_PURGE');



debug('begin add_signature: FA_APPSTECH_SA_ATK_SYSADMIN_SOA_TASKS_AVG_ARCH_DAILY');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '25587',
      p_sig_id                 => 'FA_APPSTECH_SA_ATK_SYSADMIN_SOA_TASKS_AVG_ARCH_DAILY',
      p_sig_sql                => 'SELECT  TRUNC(AVG(tt.v_archived_each_day),2) COUNT,
 ''avg_archive_per_day'' tablename 
FROM  (SELECT  COUNT(*) v_archived_each_day 
       FROM (SELECT  SYSTEMSTRING1 AS archivedon 
             FROM (SELECT  wft.roottaskid,
                    wft.taskid,
                    wft.SYSTEMSTRING1,
                    SUM(CASE WHEN (wft.state NOT IN(''ERRORED'', ''STALE'', ''WITHDRAWN'', ''EXPIRED'') AND state IS NOT NULL) THEN 1 ELSE 0 END) non_terminal_siblings 
                   FROM    fa_fusion_soainfra.wftask_sec wft 
                   GROUP BY wft.roottaskid,wft.taskid,wft.SYSTEMSTRING1) 
                  WHERE   non_terminal_siblings = 0) 
             WHERE   to_date(archivedon, ''RRRR-MM-DD'') > TRUNC(sysdate - 30) 
             GROUP BY to_date(archivedon, ''RRRR-MM-DD'')) tt',
      p_title                  => 'Average num tasks archived per day',
      p_fail_condition         => 'NRS',
      p_problem_descr          => 'Average number tasks archived per day',
      p_solution               => 'BPM Archiving process
https://confluence.oraclecorp.com/confluence/display/SE/HWF+SE+Knowledge+Base',
      p_success_msg            => 'Archiving process in place',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('I','W'),
      p_print_sql_output       => nvl('Y','RS'),
      p_limit_rows             => nvl('Y','Y'),
   ...
   l_info.delete;
debug('end add_signature: FA_APPSTECH_SA_ATK_SYSADMIN_SOA_TASKS_AVG_ARCH_DAILY');



debug('begin add_signature: FA_APPSTECH_SA_ATK_SYSADMIN_SOA_TSK_FETCH');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '25620',
      p_sig_id                 => 'FA_APPSTECH_SA_ATK_SYSADMIN_SOA_TSK_FETCH',
      p_sig_sql                => 'select PREFERENCENAME from FA_FUSION_SOAINFRA.WFUserPreference 
where preferencename=''usrPrfmaxPageRows''
and PREFERENCEVALUE > 20',
      p_title                  => 'Number of tasks per fetch',
      p_fail_condition         => 'RS',
      p_problem_descr          => '" Number of tasks per fetch" does not have recommended value of 20 for some users.',
      p_solution               => 'Please check user inbox configuration.
Our recommendation is 20 tasks per fetch. 
Edit Inbox Settings -> Number of tasks per fetch ',
      p_success_msg            => '" Number of tasks per fetch" is in recommended range.',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('W','W'),
      p_print_sql_output       => nvl('Y','RS'),
      p_limit_rows             => nvl('Y','Y'),
   ...
   l_info.delete;
debug('end add_signature: FA_APPSTECH_SA_ATK_SYSADMIN_SOA_TSK_FETCH');



debug('begin add_signature: FA_APPSTECH_SA_ATK_SYSADMIN_SOA_TERMINAL_TASKS_30_180');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '25641',
      p_sig_id                 => 'FA_APPSTECH_SA_ATK_SYSADMIN_SOA_TERMINAL_TASKS_30_180',
      p_sig_sql                => 'SELECT COUNT(task.taskid) term_30_180_active_comp
FROM FA_FUSION_SOAINFRA.wftask task
WHERE (task.state IS NULL
      OR task.state IN ( ''DELETED'', ''ERRORED'', ''EXPIRED'', ''STALE'', ''WITHDRAWN''))
    AND (task.createddate < sysdate - 30 AND task.createddate >= sysdate - 180)
    AND (task.updateddate < sysdate - 30 AND task.updateddate >= sysdate - 180)
    AND NOT EXISTS 
       (SELECT task.taskid FROM FA_FUSION_SOAINFRA.composite_instance ci WHERE task.ecid = ci.ecid)',
      p_title                  => 'Terminal tasks between 30 and 180 days with composite still active',
      p_fail_condition         => 'NRS',
      p_problem_descr          => 'Count of terminal tasks between 30 and 180 days old with composite still active',
      p_solution               => 'Count of terminal tasks between 30 and 180 days old with composite still active
 https://confluence.oraclecorp.com/confluence/display/SE/HWF+SE+Knowledge+Base#HWFSEKnowledgeBase-CheckForPurgeJobs',
      p_success_msg            => 'Count of terminal tasks between 30 and 180 days old with composite still active',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('I','W'),
      p_print_sql_output       => nvl('Y','RS'),
      p_limit_rows             => nvl('Y','Y'),
   ...
   l_info.delete;
debug('end add_signature: FA_APPSTECH_SA_ATK_SYSADMIN_SOA_TERMINAL_TASKS_30_180');



debug('begin add_signature: FA_APPSTECH_SA_ATK_SYSADMIN_SOA_PURGE_180_WAITING_ARCHIVE');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '25644',
      p_sig_id                 => 'FA_APPSTECH_SA_ATK_SYSADMIN_SOA_PURGE_180_WAITING_ARCHIVE',
      p_sig_sql                => 'SELECT COUNT(task.taskid) purge_180_waiting_archive
FROM  FA_FUSION_SOAINFRA.wftask task
WHERE (task.state IS NULL OR task.state IN ( ''DELETED'', ''ERRORED'', ''EXPIRED'', ''STALE'', ''WITHDRAWN'' ))
      AND task.createddate < sysdate - 180
      AND task.updateddate < sysdate - 180
      AND NOT EXISTS (
        SELECT task2.parenttaskid
        FROM FA_FUSION_SOAINFRA.wftask task2
        WHERE task.taskid = task2.parenttaskid
          AND task2.state IN ( ''ASSIGNED'', ''ALERTED'', ''INFO_REQUESTED'', ''OUTCOME_UPDATED'', ''SUSPENDED''))
      AND task.systemstring1 IS NULL
',
      p_title                  => 'Count tasks eligible for purge, waiting to be archived',
      p_fail_condition         => 'NRS',
      p_problem_descr          => 'Count eligible for purge disregarding composite state, waiting to be archived',
      p_solution               => 'Count eligible for purge disregarding composite state, waiting to be archived
 https://confluence.oraclecorp.com/confluence/display/SE/HWF+SE+Knowledge+Base#HWFSEKnowledgeBase-CheckForPurgeJobs',
      p_success_msg            => 'Count tasks eligible for purge waiting to be archived',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('I','W'),
      p_print_sql_output       => nvl('Y','RS'),
      p_limit_rows             => nvl('Y','Y'),
   ...
   l_info.delete;
debug('end add_signature: FA_APPSTECH_SA_ATK_SYSADMIN_SOA_PURGE_180_WAITING_ARCHIVE');



debug('begin add_signature: FA_APPSTECH_SA_ATK_SYSADMIN_SOA_EVENTS');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '25703',
      p_sig_id                 => 'FA_APPSTECH_SA_ATK_SYSADMIN_SOA_EVENTS',
      p_sig_sql                => 'select count(*)  from FA_FUSION_SOAINFRA.DLV_MESSAGE where state=0',
      p_title                  => 'Business events that are not consumed by any subscriber',
      p_fail_condition         => '[count(*)] > [500000]',
      p_problem_descr          => 'Count of business events that are not consumed by any subscriber. This can lead to performance in SOA processing events.',
      p_solution               => '[1523701.1]',
      p_success_msg            => 'The number of unprocessed business events is proper.',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('W','W'),
      p_print_sql_output       => nvl('Y','RS'),
      p_limit_rows             => nvl('Y','Y'),
   ...
   l_info.delete;
debug('end add_signature: FA_APPSTECH_SA_ATK_SYSADMIN_SOA_EVENTS');



debug('begin add_signature: FA_APPSTECH_SA_APPSTECH_PURGE_COUNT');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '25343',
      p_sig_id                 => 'FA_APPSTECH_SA_APPSTECH_PURGE_COUNT',
      p_sig_sql                => 'SELECT 
    COUNT(task.taskid) purge_30_waiting_archival
FROM
    FA_FUSION_SOAINFRA.wftask task
WHERE
    ( task.state IS NULL
      OR task.state IN ( ''DELETED'', ''ERRORED'', ''EXPIRED'', ''STALE'', ''WITHDRAWN'' ) )
    AND task.createddate < sysdate - 30
    AND task.updateddate < sysdate - 30
    AND NOT EXISTS (
        SELECT
            task2.parenttaskid
        FROM
    ...
    AND NOT EXISTS (
        SELECT
            task.taskid
        FROM
            FA_FUSION_SOAINFRA.composite_instance ci
        WHERE
            task.ecid = ci.ecid
    )
    AND task.systemstring1 IS NULL',
      p_title                  => 'Count of tasks to be purged',
      p_fail_condition         => 'RSGT1',
      p_problem_descr          => 'Count of tasks eligible for purge waiting to be archived',
      p_solution               => 'Count of tasks eligible for purge waiting to be archived',
      p_success_msg            => '',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('I','W'),
      p_print_sql_output       => nvl('Y','RS'),
   ...
   l_info.delete;
debug('end add_signature: FA_APPSTECH_SA_APPSTECH_PURGE_COUNT');



debug('begin add_signature: FA_APPSTECH_SA_ATK_SYSADMIN_SOA_AUTODISMISS_BY_DAY');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '25277',
      p_sig_id                 => 'FA_APPSTECH_SA_ATK_SYSADMIN_SOA_AUTODISMISS_BY_DAY',
      p_sig_sql                => 'select TO_CHAR(updateddate,''RRRR-MM-DD''), count(*) from FA_FUSION_SOAINFRA.wftask where workflowpattern=''FYI'' and versionreason=''Auto_Dismissal'' and updateddate > sysdate - 11 group by TO_CHAR(updateddate,''RRRR-MM-DD'') order by 1 desc',
      p_title                  => 'Autodismissed by day',
      p_fail_condition         => 'NRS',
      p_problem_descr          => 'Autodismissed by day',
      p_solution               => 'Autodismissed by day',
      p_success_msg            => 'Autodismissed by day',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('I','W'),
      p_print_sql_output       => nvl('Y','RS'),
      p_limit_rows             => nvl('Y','Y'),
   ...
   l_info.delete;
debug('end add_signature: FA_APPSTECH_SA_ATK_SYSADMIN_SOA_AUTODISMISS_BY_DAY');



debug('begin add_signature: FA_APPSTECH_SA_APPSTECH_FA_APPSTECH_SA_ATK_SOA_ELIGIBLE_TASKS4PURGE');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '25579',
      p_sig_id                 => 'FA_APPSTECH_SA_APPSTECH_FA_APPSTECH_SA_ATK_SOA_ELIGIBLE_TASKS4PURGE',
      p_sig_sql                => 'SELECT  COUNT(task.taskid) purge_30
FROM    FA_FUSION_SOAINFRA.wftask task
WHERE
        (
                task.state    IS NULL
        ...
        (
                SELECT  task.taskid
                FROM    FA_FUSION_SOAINFRA.composite_instance ci
                WHERE   task.ecid = ci.ecid
        )
        AND task.systemstring1 IS NOT NULL',
      p_title                  => 'Count of tasks eligible on this environment.',
      p_fail_condition         => 'NRS',
      p_problem_descr          => 'Count of tasks eligible for purge.',
      p_solution               => 'Count of eligible tasks for purge from past 30 days.',
      p_success_msg            => '',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('I','W'),
      p_print_sql_output       => nvl('Y','RS'),
   ...
   l_info.delete;
debug('end add_signature: FA_APPSTECH_SA_APPSTECH_FA_APPSTECH_SA_ATK_SOA_ELIGIBLE_TASKS4PURGE');



debug('begin add_signature: FA_APPSTECH_SA_ATK_SYSADMIN_SOA_ARCHIVED_PERDAY_FOR_10DAYS');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '25762',
      p_sig_id                 => 'FA_APPSTECH_SA_ATK_SYSADMIN_SOA_ARCHIVED_PERDAY_FOR_10DAYS',
      p_sig_sql                => 'select to_date(archivedon,''RRRR-MM-DD''), count(*) v_archived_each_day from FA_FUSION_SOAINFRA.wftask_view where to_date(archivedon,''RRRR-MM-DD'') > trunc(SYSDATE-11) GROUP BY to_date(archivedon,''RRRR-MM-DD'') order by 1 desc',
      p_title                  => 'Archived per day for last 10 days',
      p_fail_condition         => 'NRS',
      p_problem_descr          => 'Archived per day for last 10 days',
      p_solution               => 'Archived per day for last 10 days',
      p_success_msg            => 'Archived per day for last 10 days',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('I','W'),
      p_print_sql_output       => nvl('Y','RS'),
      p_limit_rows             => nvl('Y','Y'),
   ...
   l_info.delete;
debug('end add_signature: FA_APPSTECH_SA_ATK_SYSADMIN_SOA_ARCHIVED_PERDAY_FOR_10DAYS');



debug('begin add_signature: FA_APPSTECH_SA_ATK_SYSADMIN_SOA_INDEX');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '29954',
      p_sig_id                 => 'FA_APPSTECH_SA_ATK_SYSADMIN_SOA_INDEX',
      p_sig_sql                => 'select OBJECT_ID from all_objects where owner=''FA_FUSION_SOAINFRA'' AND OBJECT_TYPE=''INDEX'' AND OBJECT_NAME =''WFTASKHISTORY_FLOW_ID_I''',
      p_title                  => 'Verify WFTASKHISTORY_FLOW_ID_I index',
      p_fail_condition         => 'NRS',
      p_problem_descr          => 'WFTASKHISTORY_FLOW_ID_I is not created',
      p_solution               => 'Check to see if WFTASKHISTORY_FLOW_ID_I index is created',
      p_success_msg            => 'WFTASKHISTORY_FLOW_ID_I index is created',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('W','W'),
      p_print_sql_output       => nvl('Y','RS'),
      p_limit_rows             => nvl('Y','Y'),
   ...
   l_info.delete;
debug('end add_signature: FA_APPSTECH_SA_ATK_SYSADMIN_SOA_INDEX');



debug('begin add_signature: APPSTECH_SYSADMIN_SOA_ARCHIVE');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '18666',
      p_sig_id                 => 'APPSTECH_SYSADMIN_SOA_ARCHIVE',
      p_sig_sql                => 'select
/*Analyser*/
requestid,
submission,
...
definition
FROM FUSION.ESS_REQUEST_HISTORY
where definition like ''%%BPMSMC%%'' 
and (processstart > (SELECT sysdate-1 FROM dual))
order by 2,3 desc',
      p_title                  => 'Verify if SOA archive jobs are running',
      p_fail_condition         => 'NRS',
      p_problem_descr          => 'One or more of the SOA archival processes are not running',
      p_solution               => '[2691371.1] Fusion System Administration SOA Known Issues in R13

One or more of the SOA archival processes are not running',
      p_success_msg            => 'SOA archival processes status is',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('I','W'),
      p_print_sql_output       => nvl('Y','RS'),
      p_limit_rows             => nvl('Y','Y'),
   ...
   l_info.delete;
debug('end add_signature: APPSTECH_SYSADMIN_SOA_ARCHIVE');



debug('begin add_signature: APPSTECH_SYSADMIN_SOA_PURGE');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '18653',
      p_sig_id                 => 'APPSTECH_SYSADMIN_SOA_PURGE',
      p_sig_sql                => 'SELECT /*Analyzer*/ owner ,job_name,start_date,repeat_interval, state ,
 last_start_date, job_action,next_run_date 
FROM DBA_SCHEDULER_JOBS WHERE job_name 
LIKE ''%PURGE%'' AND owner = ''FA_FUSION_SOAINFRA''',
      p_title                  => 'Check the SOA Purge jobs schedule',
      p_fail_condition         => 'NRS',
      p_problem_descr          => 'Check to see if the SOA Purge schedule processes are running or are scheduled to run accordingly. ',
      p_solution               => ' [2175534.1] Fusion Applications: SOA Purge Details of a Software as a Service (SaaS) Fusion Apps Environment',
      p_success_msg            => 'Both jobs should be running or scheduled',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('I','W'),
      p_print_sql_output       => nvl('Y','RS'),
      p_limit_rows             => nvl('Y','Y'),
   ...
   l_info.delete;
debug('end add_signature: APPSTECH_SYSADMIN_SOA_PURGE');



debug('begin add_signature: FA_APPSTECH_SA_ATK_SYSADMIN_SOA_FYI_AUTO_DISMISSAL_JOB');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '25350',
      p_sig_id                 => 'FA_APPSTECH_SA_ATK_SYSADMIN_SOA_FYI_AUTO_DISMISSAL_JOB',
      p_sig_sql                => 'SELECT /*Analyzer*/ TO_CHAR((DATE ''1970-01-01'' +(1 / 24 / 60 / 60 / 1000) * prev_fire_time), ''YYYY/MM/DD HH24:MI:SS'') fyi_previous_time,
TO_CHAR((DATE ''1970-01-01'' +(1 / 24 / 60 / 60 / 1000) * next_fire_time), ''YYYY/MM/DD HH24:MI:SS'') fyi_next_time,
SCHED_NAME,TRIGGER_NAME,JOB_NAME,START_TIME,PREV_FIRE_TIME,NEXT_FIRE_TIME,PRIORITY,TRIGGER_STATE,END_TIME
FROM FA_FUSION_SOAINFRA.SOAQTZ_TRIGGERS B
WHERE TRIGGER_NAME = ''FYITaskExpirationAgent.FYITaskExpirationDelegate''',
      p_title                  => 'Check running status of FYI Auto dismissal job (FYITaskExpirationAgent.FYITaskExpirationDelegate)',
      p_fail_condition         => 'RSGT1',
      p_problem_descr          => 'Check if the FYI Task Auto-Dismissal job is in place',
      p_solution               => 'FYI Auto Dismissal Job Not Working: 
https://confluence.oraclecorp.com/confluence/display/SE/HWF+SE+Knowledge+Base',
      p_success_msg            => 'FYI Auto Dismissal Job is scheduled',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('I','W'),
      p_print_sql_output       => nvl('Y','RS'),
      p_limit_rows             => nvl('Y','Y'),
   ...
   l_info.delete;
debug('end add_signature: FA_APPSTECH_SA_ATK_SYSADMIN_SOA_FYI_AUTO_DISMISSAL_JOB');



debug('begin add_signature: FA_APPSTECH_SA_ATK_SYSADMIN_SOA_LAST_AUTO_D');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '25544',
      p_sig_id                 => 'FA_APPSTECH_SA_ATK_SYSADMIN_SOA_LAST_AUTO_D',
      p_sig_sql                => 'SELECT MAX(UPDATEDDATE), STATE, WORKFLOWPATTERN FROM   FA_FUSION_SOAINFRA.WFTASK WHERE    VERSIONREASON = ''Auto_Dismissal'' GROUP BY    STATE, WORKFLOWPATTERN',
      p_title                  => 'Last time auto-dismissal job successfully updated a task',
      p_fail_condition         => 'NRS',
      p_problem_descr          => 'Last time auto-dismissal job successfully updated a task',
      p_solution               => 'Last time auto-dismissal job successfully updated a task',
      p_success_msg            => 'Last time auto-dismissal job successfully updated a task',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('I','W'),
      p_print_sql_output       => nvl('Y','RS'),
      p_limit_rows             => nvl('Y','Y'),
   ...
   l_info.delete;
debug('end add_signature: FA_APPSTECH_SA_ATK_SYSADMIN_SOA_LAST_AUTO_D');



debug('begin add_signature: FA_APPSTECH_SA_APPSTECH_FA_APPSTECH_SA_ATK_SOA_PURGEDETAILS');
   l_info('##SHOW_SQL##'):= 'Y';
  add_signature(
      p_sig_repo_id            => '25578',
      p_sig_id                 => 'FA_APPSTECH_SA_APPSTECH_FA_APPSTECH_SA_ATK_SOA_PURGEDETAILS',
      p_sig_sql                => 'SELECT job_name, log_date, status, actual_start_date FROM dba_scheduler_job_run_details where job_name  in (''HWF_PURGE_FA'', ''BPM_PURGE_FA_34334226'') order by actual_start_date desc',
      p_title                  => 'Information about the purge job BPM_FA and hwf_FA job.',
      p_fail_condition         => 'NRS',
      p_problem_descr          => 'Job run details of the two purge jobs scheduled on the POD.',
      p_solution               => 'Job run details of the two purge jobs scheduled on the POD.',
      p_success_msg            => '',
      p_print_condition        => nvl('ALWAYS','ALWAYS'),
      p_fail_type              => nvl('I','W'),
      p_print_sql_output       => nvl('Y','RS'),
   ...
   l_info.delete;
debug('end add_signature: FA_APPSTECH_SA_APPSTECH_FA_APPSTECH_SA_ATK_SOA_PURGEDETAILS');
...
BEGIN
  ...
  analyzer_title := regexp_replace('Fusion SaaS Service Oriented Architecture (SOA) Analyzer', '(\\S+\\s)\\1', '\\1', 1, 0, 'i');  -- EBSAF-243
  ...
  set_trace_module(null, 'parse_parameters');
    ...
    if PURPOSE is null then
        PURPOSE := 'Fusion SaaS Service Oriented Architecture (SOA) Analyzer';
        debug('Using default value for PURPOSE');
    end if;
...
start_section('Overview', 'Overview');
   set_item_result(run_stored_sig('APPSTECH_SYSADMIN_SOA_GENERIC_POD_DB_INFO'));
   set_item_result(run_stored_sig('APPSTECH_SYSADMIN_SOA_GENERIC_WHITELISTED_IP_LBAC'));
   set_item_result(run_stored_sig('APPSTECH_SYSADMIN_SOA_AGG_FYI_DANGLING'));
   set_item_result(run_stored_sig('APPSTECH_SYSADMIN_SOA_WFTASK_RETRY'));
   set_item_result(run_stored_sig('APPSTECH_SYSADMIN_SOA_COMPOSITE_DN'));
   set_item_result(run_stored_sig('APPSTECH_SYSADMIN_SOA_BRANDINGLOGO'));
   set_item_result(run_stored_sig('APPSTECH_SYSADMIN_SOA_BPM_REMINDER_PREFIX'));
   set_item_result(run_stored_sig('APPSTECH_SYSADMIN_SOA_FYI_EXPIRATION_PERIOD'));
   set_item_result(run_stored_sig('APPSTECH_SYSADMIN_SOA_GENERIC_TABLESPACE'));
   set_item_result(run_stored_sig('APPSTECH_SYSADMIN_SOA_GENERIC_TEMPSPACE'));
   set_item_result(run_stored_sig('APPSTECH_SYSADMIN_SOA_AFLOG_CHECK'));
   set_item_result(run_stored_sig('FA_APPSTECH_SA_ATK_SYSADMIN_SOA_WFTASK_COUNT'));
end_section;
debug('end section: Overview');

debug('begin section: Notifications');
start_section('Notification Status', 'Notifications');
   set_item_result(run_stored_sig('APPSTECH_SYSADMIN_SOA_UMS_DRIVER'));
   set_item_result(run_stored_sig('APPSTECH_SYSADMIN_SOA_NOTIF_DAYS'));
   set_item_result(run_stored_sig('APPSTECH_SYSADMIN_SOA_INCOMING_MSG'));
   set_item_result(run_stored_sig('APPSTECH_SYSADMIN_SOA_SYNC_BELL'));
   set_item_result(run_stored_sig('FA_APPSTECH_SA_APPSTECH_SYSADMIN_SOA_ORPHAN_TASK_PART1'));
   set_item_result(run_stored_sig('FA_APPSTECH_SA_APPSTECH_SYSADMIN_SOA_ORPHAN_TASK_PART2'));
   set_item_result(run_stored_sig('FA_APPSTECH_SA_ATK_SYSADMIN_SOA_INCOMING_EMAIL_ALERT1'));
   set_item_result(run_stored_sig('FA_APPSTECH_SA_ATK_SYSADMIN_SOA_INCOMING_EMAIL_ALERT2'));
   set_item_result(run_stored_sig('FA_APPSTECH_SA_ATK_SYSADMIN_SOA_INCOMING_EMAIL_ALERT3'));
end_section;
debug('end section: Notifications');

debug('begin section: Performance');
start_section('Performance', 'Performance');
   set_item_result(run_stored_sig('APPSTECH_SYSADMIN_SOA_EDN_QUEUE'));
   set_item_result(run_stored_sig('APPSTECH_SYSADMIN_SOA_WFTASK_STATS'));
   set_item_result(run_stored_sig('APPSTECH_SYSADMIN_SOA_WFTASK_STALE_COUNT'));
   set_item_result(run_stored_sig('FA_APPSTECH_SA_ATK_SYSADMIN_SOA_COMMON_FAULT'));
   set_item_result(run_stored_sig('FA_APPSTECH_SA_ATK_SOA_FYI_DISMISS'));
   set_item_result(run_stored_sig('FA_APPSTECH_SA_ATK_SYSADMIN_SOA_ARCHIVED_NOT_PURGED'));
   set_item_result(run_stored_sig('FA_APPSTECH_SA_ATK_SYSADMIN_SOA_N_ARCH'));
   set_item_result(run_stored_sig('FA_APPSTECH_SA_ATK_SYSADMIN_SOA_COUNT_PURGE'));
   set_item_result(run_stored_sig('FA_APPSTECH_SA_ATK_SYSADMIN_SOA_TASKS_AVG_ARCH_DAILY'));
   set_item_result(run_stored_sig('FA_APPSTECH_SA_ATK_SYSADMIN_SOA_TSK_FETCH'));
   set_item_result(run_stored_sig('FA_APPSTECH_SA_ATK_SYSADMIN_SOA_TERMINAL_TASKS_30_180'));
   set_item_result(run_stored_sig('FA_APPSTECH_SA_ATK_SYSADMIN_SOA_PURGE_180_WAITING_ARCHIVE'));
   set_item_result(run_stored_sig('FA_APPSTECH_SA_ATK_SYSADMIN_SOA_EVENTS'));
   set_item_result(run_stored_sig('FA_APPSTECH_SA_APPSTECH_PURGE_COUNT'));
   set_item_result(run_stored_sig('FA_APPSTECH_SA_ATK_SYSADMIN_SOA_AUTODISMISS_BY_DAY'));
   set_item_result(run_stored_sig('FA_APPSTECH_SA_APPSTECH_FA_APPSTECH_SA_ATK_SOA_ELIGIBLE_TASKS4PURGE'));
   set_item_result(run_stored_sig('FA_APPSTECH_SA_ATK_SYSADMIN_SOA_ARCHIVED_PERDAY_FOR_10DAYS'));
   set_item_result(run_stored_sig('FA_APPSTECH_SA_ATK_SYSADMIN_SOA_INDEX'));
end_section;
debug('end section: Performance');

debug('begin section: SOAScheduleJobs');
start_section('Schedule Jobs', 'SOAScheduleJobs');
   set_item_result(run_stored_sig('APPSTECH_SYSADMIN_SOA_ARCHIVE'));
   set_item_result(run_stored_sig('APPSTECH_SYSADMIN_SOA_PURGE'));
   set_item_result(run_stored_sig('FA_APPSTECH_SA_ATK_SYSADMIN_SOA_FYI_AUTO_DISMISSAL_JOB'));
   set_item_result(run_stored_sig('FA_APPSTECH_SA_ATK_SYSADMIN_SOA_LAST_AUTO_D'));
   set_item_result(run_stored_sig('FA_APPSTECH_SA_APPSTECH_FA_APPSTECH_SA_ATK_SOA_PURGEDETAILS'));
end_section;
debug('end section: SOAScheduleJobs');
...
begin
    if (:b_exit_code = 0) then
        ...
        :b_xml_clob := '<analyzer>' || chr(10) ||
            ...
            '<analyzer_id>4909</analyzer_id>' || chr(10) ||
            '<analyzer_version>200.10</analyzer_version>' || chr(10) ||
            '<short_name>SYSADMIN-SOA</short_name>' || chr(10) ||
